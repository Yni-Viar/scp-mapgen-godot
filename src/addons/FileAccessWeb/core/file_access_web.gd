...
signal loaded(file_name: String, data: PackedByteArray)
...
func _on_file_loaded(args: Array) -> void:
	var file_name: String = args[0]
	var data: JavaScriptObject = args[1]
	loaded.emit(file_name, JavaScriptBridge.js_buffer_to_packed_byte_array(data))
